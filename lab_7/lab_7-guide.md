...
2. cd into the lab_7/python/ directory:
    ...
    cd ~/SRv6_dCloud_Lab/lab_7/python/
    ```
3. Everything is the same as on the Rome VM with some different parameters in amsterdam.json:
    ```
    cat ./amsterdam.json
    ```
4. Amsterdam has a Linux veth pair connecting kernel forwarding to its onboard VPP instance. The VM has preconfigured ip routes (see /etc/netplan/00-installer-config.yaml) pointing to VPP via its "ams-out" interface:
    ```
    ...
    ```
5. VPP has been given a startup config which establishes IP connectivity to the network as a whole on bootup.
    ```
...
6. VPP's CLI may be invoked directly:
    ```
    ...
    ```
7. Or driven from the Linux command line:
    ```
